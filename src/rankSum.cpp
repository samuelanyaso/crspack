    ...
    if((nj0==0) || (nj1==0)){
      ...
      if(swapG){
        if(nj1==0){
          ...
          arma::imat random_value = arma::randi<arma::imat>(1, 1, arma::distr_param(0, dw0i.n_rows-1));
          rv = random_value(0, 0);
          // replace the group indicator
          dw0i.at(rv,2) = 1;
        } else if (nj0==0){

          // Generate a random sample
          arma::imat random_value = arma::randi<arma::imat>(1, 1, arma::distr_param(0, dw1i.n_rows-1));
          rv = random_value(0, 0);
          // replace the group indicator
          dw1i.at(rv,2) = 0;
        }
      }
      temp = join_vert(dw0i, dw1i);
      resMat = join_vert(resMat, temp);
    } else {
      temp = join_vert(dw0i, dw1i);
      resMat = join_vert(resMat, temp);
    }
...
arma::mat goo2C (arma::mat dw, arma::mat dw1, int m){
 ...
 List DDstatV2C (arma::mat dw, bool iICGs=true, bool get_var=true){
   ...
   tmp4 = (tmp0 + 2*nv1 % tmp1) / (4 * nv1); // second term of T (see page 5 of scratch paper)
   tmp5 = (tmp0 + 2*tmp1) / (2 * nv1); // first term of T (see page 5 of scratch paper)
...
// [[Rcpp::export]]
/* Compute the V statistic. Takes a matrix as input.
 - first col is the cluster id
 - second column is the response
 - third column is the continuous covariate */
